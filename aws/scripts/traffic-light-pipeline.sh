...
if [[ "$COMMIT_MSG" != *"[$CI_TOOL]"* ]]; then
  echo "❌ Skipping - not $CI_TOOL job (commit: '$COMMIT_MSG')"
  exit 0
fi
...
if [[ "$COMMIT_MSG" == *"[release]"* ]] && [[ "$BRANCH_REF" != "refs/heads/main" ]]; then
  echo "→ Creating release PR..."
  
  # Get current branch name
  BRANCH_NAME=$(echo "$BRANCH_REF" | sed 's|refs/heads/||' || echo "development")
